...
class MenuPage(webapp.RequestHandler):
	def get(self, menu_key):
		...
		if not users.get_current_user().email()=="test@admin.com" :
			if menu.author:
				if not users.get_current_user()==menu.author:
					self.redirect('/admin/')
					return	
			else:
				self.redirect('/admin/')
	...
	def put(self, menu_key):
		menu = Menu.get(menu_key)
		if users.get_current_user().email()=="test@admin.com":
			...
			if menu.author:
				if menu.author== users.get_current_user():
					if self.request.get('name'):
						menu.name = self.request.get('name')
					menu.put()			
					memcache.flush_all()
